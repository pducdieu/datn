    ...
    <body>
        ...
        if (isset($_GET['action']) && $_GET['action'] == 'edit') {
            if (isset($_POST['id']) && !empty($_POST['id']) ) {
                ...
                $result_detail = mysqli_query($con, "UPDATE `orders_detail` SET `time` = " . time() . " WHERE `order_id` = $id ");
                if($_POST['status']==2)
                {
                    $kq= mysqli_query($con, "SELECT  `product_id`,  `quantity` FROM `orders_detail` WHERE `order_id`= $id");
                    
                    while($upl = mysqli_fetch_assoc($kq))
                    {
                        $result = mysqli_query($con, "UPDATE `product` SET `sltk`=`sltk`-".$upl['quantity'].",`sldb`=`sldb`+".$upl['quantity']."  WHERE `id` = " . $upl['product_id'] . " ");
                    }
                }
                if (!$result) {
            ...
            if (!empty($user)) {
                ...
                <div id="edit_user" class="box-content">
                    <h1>Cập nhật trạng thái tài khoản "<?= $user['id'] ?>"</h1>
